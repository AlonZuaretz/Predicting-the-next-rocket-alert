...
def calc_pos_weight(dl):
    ...
    return torch.tensor(pos_weights, dtype=torch.float), total_ones, total_zeros
...
def train_model(articles_NN, threats_NN, combined_NN, dl_train, dl_test, device, num_epochs, lr, load_run=None):
    ...
    if load_run is not None:
        ...
        if os.path.exists(run_folder):
            articles_NN.load_state_dict(torch.load(os.path.join(run_folder, 'last_model.pth'))['articles_NN'])
            threats_NN.load_state_dict(torch.load(os.path.join(run_folder, 'last_model.pth'))['threats_NN'])
            combined_NN.load_state_dict(torch.load(os.path.join(run_folder, 'last_model.pth'))['combined_NN'])
